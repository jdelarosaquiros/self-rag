...
def call_model_rerank_w_scores_batch(
    ...
    if do_retrieve is True:
        evidence_augmented_inputs = [
            prompt + "[Retrieval]<paragraph>{0}</paragraph>".format(para)
    ...
    if len(results) == 1:
        ...
        print("Retrieve:", do_retrieve)
        return postprocessed_pred, "no_retrieval", results, do_retrieve
    else:
        answer2score = {}
        if closed is True:
        ...
        return best_option, best_path, results, do_retrieve
...
def main():
    ...
    for i, row in tqdm(enumerate(input_data)):
        ...
        pred, pred_path, results, do_retrieve = generate(
            prompt,
            evidences,
